...
CREATE INDEX promocodes_used_at_idx ON public.promocodes (used_at);

GRANT ALL ON public.promocodes TO service_role;
ALTER TABLE public.promocodes ENABLE ROW LEVEL SECURITY;

-- No authenticated policies: redeem/list only via service-role server functions.
CREATE POLICY "Admin read promocodes"
  ON public.promocodes FOR SELECT TO authenticated
  USING (public.is_admin_caller());

CREATE TABLE public.promo_redeem_attempts (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX promo_redeem_attempts_ip_created_idx
