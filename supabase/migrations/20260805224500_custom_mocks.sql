-- Custom Mock Builder: store AI-generated Economics mocks per user.
CREATE TABLE public.custom_mocks (
  ...
  CONSTRAINT custom_mocks_question_count_check
    CHECK (question_count > 0 AND question_count <= 40),
  CONSTRAINT custom_mocks_duration_check
    CHECK (duration_minutes > 0)
);

CREATE INDEX custom_mocks_user_idx
  ON public.custom_mocks (user_id, created_at DESC);
