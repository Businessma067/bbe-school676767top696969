#!/usr/bin/env python3
"""Rewrite Part 3 Solve steps for math-11-41..80 to Ch13 one-font style."""
...
# Hand-crafted Part 3 bodies (numbered steps only; Answer line stays in file).
PART3: dict[int, str] = {
    41: r"""**1.** Discount factor: $(1.05)^{-1} = 1/1.05 \\approx 0.9524$.
...
**5.** At $r = 0\\%$: $PDV = 8,000 \\times (1.00)^{-1} = \\$8,000$.""",
    42: r"""**1.** $rt = 0.06 \\times 3 = 0.18$, so $e^{-0.18} \\approx 0.8353$.
...
**5.** For $t = 6$: $rt = 0.36$, $e^{-0.36} \\approx 0.6977$, so $PDV = 12,000 \\times 0.6977 \\approx \\$8,372.11$.""",
    43: r"""**1.** $(1.07)^{8} \\approx 1.718186$, so $(1.07)^{-8} \\approx 0.5820$.
...
**5.** At $r = 0$: both formulas reduce to $K \\times 1 = \\$45,000$.""",
    44: r"""**1.** $rt = 0.045 \\times 5 = 0.225$, so $e^{-0.225} \\approx 0.7985$.
...
**5.** For $t = 10$: $rt = 0.45$, $e^{-0.45} \\approx 0.6376$, so $A = 150,000 \\times 0.6376 \\approx \\$95,644.20$.""",
    45: r"""**1.** $K/PDV = 25,000/18,500 \\approx 1.3514$.
...
**6.** Compare continuous $t \\approx 5.02$ years vs annual $t \\approx 5.17$ years.""",
    46: r"""**1.** $PDV/K = 27,000/60,000 = 0.45$.
...
**5.** For $t = 24$ with the same discount factor $0.45$: $r = -\\ln(0.45)/24 = 0.798508/24 \\approx 0.0333 = 3.33\\%$.""",
    47: r"""**1.** $PDV_1 = 40,000 \\times (1.05)^{-2} = 40,000/1.1025 \\approx \\$36,281.18$.
...
**5.** Continuous: $PDV_1 = 40,000 \\times e^{-0.10} \\approx \\$36,193.48$; $PDV_2 = 65,000 \\times e^{-0.25} \\approx \\$50,622.07$; combined $\\approx \\$86,815.55$.""",
    48: r"""**1.** $(1.06)^{3} \\approx 1.191016$, so PDV of Option B: $25,500/1.191016 \\approx \\$21,410.30$.
...
**5.** $f(25) = P(25)e^{-0.08 \\times 25} = 5,000(27)^{2}e^{-2} = 3,645,000 \\times 0.135335 \\approx \\$493,296.10$.""",
    50: r"""**1.** $rt_1 = 0.055 \\times 4 = 0.22$, $e^{-0.22} \\approx 0.8025$, so $PDV_1 = 18,000 \\times 0.8025 \\approx \\$14,445.34$.
...
**5.** At $r = 0\\%$: $e^{0} = 1$ for both terms, so combined $PDV = 18,000 + 30,000 = \\$48,000$.""",
    51: r"""**1.** $rt = 0.05 \\times 7 = 0.35$, $e^{-0.35} \\approx 0.704688$, so $PDV \\approx 50,000 \\times 0.704688 \\approx \\$35,234.40$.
...
**4.** At $t = 3$: continuous $PDV = 50,000 \\times e^{-0.15} \\approx \\$43,035.40$; annual-equivalent $PDV = 50,000 \\times (1.051271)^{-3} \\approx \\$43,035.40$ (identical).""",
    52: r"""**1.** $PV_1 = 42,000 \\times (1.06)^{-3} = 42,000/1.191016 \\approx \\$35,264.01$.
...
**5.** At $r = 8\\%$: $K_2 = 64,735.99 \\times (1.08)^{6} = 64,735.99 \\times 1.586874 \\approx \\$102,727.88$.""",
    53: r"""**1.** $rt = 0.065 \\times 4 = 0.26$, so $e^{-0.26} \\approx 0.7711$.
...
**5.** At $t = 2$: $rt = 0.13$, $e^{-0.13} \\approx 0.8781$, so $K = 35,000/0.8781 \\approx \\$39,858.99$.""",
    54: r"""**1.** $P'(t) = 0.05 \\times 40,000e^{0.05t} = 0.05P(t)$, so the condition $P'(t^*) = rP(t^*)$ would require $0.05P(t^*) = 0.08P(t^*)$, impossible for $P(t^*) > 0$.
...
**5.** If $r = 4\\% < 5\\%$ growth rate, $f(t) = 40,000e^{(0.05-0.04)t} = 40,000e^{0.01t}$, which increases without bound in $t$.""",
    55: r"""**1.** $0.09 \\times 520,000 = \\$46,800$, matching the given $P'(t^*)$ exactly.
...
**5.** $-1,092 < 0$, so the second-order condition holds.""",
    56: r"""**1.** $6,000(t + 4) = 0.09 \\times 3,000(t+4)^{2} = 270(t+4)^{2}$; dividing by $(t + 4)$ gives $6,000 = 270(t + 4)$, so $t + 4 = 22.2222$, i.e., $t^* \\approx 18.22$ years.
...
**8.** General relation for this $P(t)$: $t^* + 4 = 2/r$, so $t^* = 2/r - 4$.""",
    57: r"""**1.** $rt_1 = 0.11 \\times 2.5 = 0.275$, $e^{-0.275} \\approx 0.759572$, so $PV_1 = 250,000 \\times 0.759572 \\approx \\$189,893.03$.
...
**5.** At $r = 0\\%$: $e^{0} = 1$, so combined $PDV = 250,000 + 40,000 = \\$290,000$.""",
    58: r"""**1.** $PDV/K = 2,000,000/3,200,000 = 0.625$.
...
**5.** For $t = 9$ with the same discount factor $0.625$: $r = -\\ln(0.625)/9 = 0.470004/9 \\approx 0.052223 = 5.22\\%$.""",
    59: r"""**1.** $2A(t + k) = r A(t + k)^{2}$; dividing both sides by $A(t + k)$ (nonzero) gives $2 = r(t + k)$, so $t^* = 2/r - k$.
...
**5.** Combined PDV: $20,109.60 + 24,713.09 \\approx \\$44,822.69$.""",
    61: r"""**1.** Year-$2$ revenue: $a \\times k = 50\\times1.10 = \\$55.00$ million.
...
**6.** Difference from actual growth total: $305.255 - 250.00 = \\$55.26$ million (not $\\$60.00$ million).""",
    62: r"""**1.** $|k| = 0.5 < 1$, so the series converges.
...
**5.** If $k = 1.5$, then $|k| = 1.5 \\ge 1$, so the series diverges and has no finite sum at all.""",
    63: r"""**1.** $|k| = 0.90 < 1$, so the infinite series converges.
...
**6.** If $k = 1.10$, $|k| = 1.10 \\ge 1$, so the infinite series diverges by definition.""",
    64: r"""**1.** $t = 18,000,000 / 300,000 = 60$ years.
...
**7.** Difference: $3,773,368 - 3,000,000 = 773,368$ tons, which is less than $1,000,000$ tons.""",
    65: r"""**1.** Year-$2$ output: $a \\times k = 180\\times0.97 = 174.6$ million tons.
...
**7.** $s_{20} = 180 \\times (1 - 0.5438)/0.03 = 180 \\times 15.207 \\approx 2,737.3$ million tons, which is far less than $6,000$ million tons.""",
    66: r"""**1.** $|k| = |-0.5| = 0.5 < 1$, so the series converges.
...
**5.** For $k = -1$: $s_n = a$ (odd $n$) or $s_n = 0$ (even $n$) — the partial sums oscillate forever and never settle on a limit.""",
    67: r"""**1.** Equal payments: $s_{15} = 12 \\times 15 = \\$180.00$ million.
...
**5.** The general formula's denominator is $(k - 1)$; at $k = 1$ this denominator is $0$, so the formula is undefined and the $k = 1$ case must instead use $s_n = a \\times n$.""",
    68: r"""**1.** Payment $2$: $a \\times k = 15,000\\times0.88 = \\$13,200.00$.
...
**6.** With $k = 0.95$: infinite sum $= 15,000/(1 - 0.95) = 15,000/0.05 = \\$300,000.00$, which is larger, not smaller, than the $\\$125,000.00$ obtained at $k = 0.88$.""",
    69: r"""**1.** $1.08^{12} \\approx 2.518170$.
...
**5.** Combined value: $\\$39.72$ million $+ \\$66.36$ million $= \\$106.08$ million, which is not less than $\\$100$ million.""",
    71: r"""**1.** Hypothetical: if month-$1$ cost were $\\$1,000$, month-$2$ cost: $1,000\\times1.15 = \\$1,150.00$.
...
**6.** Flat monthly figure: $58,000/6 = \\$9,666.67$, which is indeed greater than $\\$6,625.74$.""",
    72: r"""**1.** Year-$2$ payout: $a \\times k = 500,000\\times0.98 = \\$490,000.00$.
...
**7.** Half of the original infinite sum $= 25,000,000.00/2 = \\$12,500,000.00$; since $10,000,000.00 < 12,500,000.00$, it is NOT more than half.""",
    73: r"""**1.** Year-$2$ budget: $a \\times k = 200,000\\times1.12 = \\$224,000.00$.
...
**5.** $1.08^{10} = 2.158924997$; $s_{10}$ (at $k=1.08$) $= 200,000 \\times (2.158924997-1)/0.08 = 200,000 \\times 14.486562 = \\$2,897,312.49$, which is below $\\$3,000,000$.""",
    74: r"""**1.** Grant $2$: $a \\times k = 50,000\\times0.96 = \\$48,000.00$.
...
**6.** Infinite sum at $k = 0.90$: $50,000/(1 - 0.90) = 50,000/0.10 = \\$500,000.00$; half of $\\$1,250,000.00$ is $\\$625,000.00$, and $\\$500,000.00 < \\$625,000.00$.""",
    75: r"""**1.** Quarter-$2$ yield: $a \\times k = 10,000\\times0.98 = 9,800.00$ lbs.
...
**6.** Infinite (hypothetical) sum: $10,000/(1 - 0.98) = 10,000/0.02 = 500,000.00$ lbs, which is MORE than the $166,196.01$ lbs $20$-quarter total, not less.""",
    76: r"""**1.** $1.06^{8} = 1.593848075$; $s_{A,8} = 80,000 \\times (1.593848075-1)/0.06 = 80,000 \\times 9.897468 = \\$791,797.43$.
...
**6.** Difference (B $-$ A): $\\$815,382.06 - \\$791,797.43 = \\$23,584.63$, which is NOT more than $\\$30,000$.""",
    77: r"""**1.** $4^{1.5} = 4 \\times \\sqrt{4} = 4 \\times 2 = 8$; $a_4 = 5,000/8 = \\$625.00$ (not $\\$650.00$).
...
**5.** $p = 0.5 \\le 1$, so by the $p$-series rule $\\sum 5,000/n^{0.5}$ diverges, even though $a_n = 5,000/\\sqrt{n} \\to 0$ as $n \\to \\infty$.""",
    78: r"""**1.** $1.01^{12} = 1.126825030$; $s_{\\mathrm{rev},12} = 150,000 \\times (1.126825030-1)/0.01 = 150,000 \\times 12.682503 = \\$1,902,375.45$.
...
**8.** $1.03^{20} = 1.806111235$; $s_{\\mathrm{cost},20} = 120,000 \\times (0.806111235)/0.03 = \\$3,224,444.94$.""",
    79: r"""**1.** Infinite recession sum: $2,400,000/(1 - 0.94) = 2,400,000/0.06 = \\$40,000,000.00$.
...
**5.** Year-$7$ recovery cash flow: $2,400,000 \\times 1.06^{6} = 2,400,000 \\times 1.418519 = \\$3,404,445.87$ (not $\\$2,900,000.00$).""",
    80: r"""**1.** Tranche $1$: $s = 25,000 \\times 9 = \\$225,000.00$.
...
def audit_body(n: int, body: str) -> list[str]:
    issues = []
    if "×" in body:
        issues.append(f"{n}: unicode times")
    if re.search(r"\\{3,}approx", body):
        issues.append(f"{n}: triple-backslash approx")
    if re.search(r"\d\$, \$", body):
        issues.append(f"{n}: broken thousands")
    if re.search(r"\$At\s", body):
        issues.append(f"{n}: $At scar")
    for line in body.split("\n"):
        m = re.match(r"^(\*\*\d+\.\*\*)\s+(.*)$", line)
        if m and digits_outside_math(m.group(2)):
            issues.append(f"{n}: digits outside math: {m.group(2)[:100]}")
        if m and re.search(r"\$(?:\\mathrm\{)?PV_?\d\}?\s*:", m.group(2)):
            issues.append(f"{n}: PV colon form: {m.group(2)[:80]}")
        if m and re.search(r"needed\s*=", m.group(2), re.I):
            issues.append(f"{n}: needed inside: {m.group(2)[:80]}")
    return issues
...
def main() -> None:
    ...
    for n, body in PART3.items():
        # Normalize: body uses \\ in raw strings → file needs \\ for each backslash pair in TS
        # Our PART3 values are raw strings with \\approx etc — correct for .ts source.
        tid = f"math-11-{n}"
        m = re.search(rf"(id: `{tid}`,.*?)\*\*Part 3: Solve\.\*\*\n\n(.*?)\n\n\*\*Answer\.\*\*", src, re.S)
        if not m:
            raise SystemExit(f"Could not find Part 3 for {tid}")
        old = m.group(2)
        if old != body:
            touched += 1
        src = src[: m.start(2)] + body + src[m.end(2) :]
        all_issues.extend(audit_body(n, body))

    # Global scar fixes only within replaced region already handled; still fix any remaining in range
    src = re.sub(r"\\{3,}(approx|times|%)", r"\\\\\1", src)
    prev = None
    while prev != src:
        prev = src
    ...
    for i in all_issues[:40]:
        print(" ", i)

    # Print 52 and 53
    for n in (52, 53):
        tid = f"math-11-{n}"
        m = re.search(rf"id: `{tid}`,.*?\*\*Part 3: Solve\.\*\*\n\n(.*?)\n\n\*\*Answer\.\*\*", PATH.read_text(encoding="utf-8"), re.S)
        print("=" * 40, tid)
