...
const easy = JSON.parse(
  fs.readFileSync('C:/Users/bubli/Projects/bbe-school-fixed/textbook/output/_ch11_easy_current.json', 'utf8')
);
const beforeMap = Object.fromEntries(easy.map((t) => [t.id, t]));

/** Content-trimmed overviews for tasks over target (no Answer lines). */
const trimmed = {
...
function replaceOverview(id, newOv) {
  ...
  // Ensure we are inside this task: next id boundary
  ...
  return { oldLen: oldOv.length, newLen: newOv.length, hadAnswer: /\*\*Answer\.\*\*/.test(oldOv) };
}
...
// 1) Content-trim the over-target set
for (const [id, ov] of Object.entries(trimmed)) {
  const info = beforeMap[id];
  const target = info.diff === '1/5' ? [700, 1100] : [900, 1200];
  if (ov.length < target[0] || ov.length > target[1]) {
    console.warn('OUT OF BAND', id, ov.length, target);
  }
  const res = replaceOverview(id, ov);
  if (info.diff === '1/5') trimmed15++;
  else trimmed25++;
  if (examples.length < 2) {
    examples.push({ id, diff: info.diff, before: res.oldLen, after: res.newLen });
  }
  console.log('TRIM', info.diff, id, res.oldLen, '->', res.newLen);
}

// 2) Strip Answer lines from remaining easy tasks (in-range after strip)
for (const t of easy) {
  if (trimmed[t.id]) continue;
  if (!t.hasAnswer) continue;
  const newOv = t.stripped;
  const res = replaceOverview(t.id, newOv);
  if (t.diff === '1/5') answerStrip15++;
  else answerStrip25++;
  console.log('STRIP', t.diff, t.id, res.oldLen, '->', res.newLen);
}

const final = nl === '\r\n' ? out.replace(/\n/g, '\r\n') : out;
fs.writeFileSync(path, final);

// Verify immediately from what we wrote
const check = fs.readFileSync(path, 'utf8').replace(/\r\n/g, '\n');
...
for (let i = 1; i < parts.length; i++) {
  ...
  if (/\*\*Answer\.\*\*/.test(ov)) answerHits++;
  const target = diffM[1] === '1/5' ? 1100 : 1200;
  if (ov.length > target) {
    over++;
    console.log('STILL OVER', diffM[1], id, ov.length);
  }
  lens.push({ id, diff: diffM[1], len: ov.length });
}

console.log('\n=== SUMMARY ===');
console.log('Content-trimmed 1/5:', trimmed15);
console.log('Content-trimmed 2/5:', trimmed25);
console.log('Answer-stripped only 1/5:', answerStrip15);
console.log('Answer-stripped only 2/5:', answerStrip25);
console.log('Examples:', examples);
console.log('Answer lines left in easy overviews:', answerHits);
console.log('Still over target:', over);

fs.writeFileSync(
  'C:/Users/bubli/Projects/bbe-school-fixed/textbook/output/_ch11_trim_summary.json',
  JSON.stringify({ trimmed15, trimmed25, answerStrip15, answerStrip25, examples, lens }, null, 2)
);
