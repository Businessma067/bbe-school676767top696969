...
// Normalize to LF for matching; restore CRLF at end if file used it
...
function replaceInTask(id, oldStr, newStr) {
  ...
  const end = next < 0 ? Math.min(src.length, start + 20000) : next;
  const chunk = src.slice(start, end);
  if (!chunk.includes(oldStr)) {
    console.error("NOT FOUND in", id, ":", JSON.stringify(oldStr.slice(0, 90)));
    return false;
  }
  src = src.slice(0, start) + chunk.replace(oldStr, newStr) + src.slice(end);
  return true;
}
...
function R(id, a, b) {
  if (replaceInTask(id, a, b)) {
    n++;
    console.log("OK", id, a.slice(0, 55).replace(/\n/g, " "));
  }
}

// ---- math-11-1 A tactical + Part 3 ----
R(
  "math-11-1",
...
// ---- math-11-11 (statement + tac + part3) ----
R(
  "math-11-11",
...
// ---- math-11-25: fix broken $ escaping + tactical + part3 ----
R(
  "math-11-25",
...
R(
  ...
  `**5.** $S(2) \\approx \\$103,946.56$, which does not exceed $\\$105,000$.`,
);

if (usedCRLF) src = src.replace(/\n/g, "\r\n");
