...
function extractP3(block) {
  const p = block.indexOf("**Part 3: Solve.**");
  const a = block.indexOf("**Answer.**");
  if (p < 0 || a < 0) throw new Error("missing part3/answer");
  return { p, a, text: block.slice(p, a) };
}

/** Rewrite maps: taskId -> full Part 3 text (including header, ending with blank line before Answer) */
const REWRITES = {};

REWRITES[1] = `**Part 3: Solve.**
...
// Gold-example scars on math-11-52 (same Ch13 rules)
REWRITES[52] = `**Part 3: Solve.**
...
for (const id of Object.keys(REWRITES).map(Number).sort((a, b) => a - b)) {
  const { start, end, block } = getBlock(id);
  const { p, a, text: oldP3 } = extractP3(block);
  let newP3 = REWRITES[id].replace(/\\/g, "\\\\");
  // Match file line endings
  if (block.includes("\r\n")) newP3 = newP3.replace(/\n/g, "\r\n");
  if (oldP3 === newP3) {
    console.log(`math-11-${id}: unchanged`);
    continue;
  }
  // rebuild: everything before p3 in block + new p3 + answer onward
  const newBlock = block.slice(0, p) + newP3 + block.slice(a);
  // accumulate via slicing original
  parts.push(s.slice(cursor, start));
  parts.push(newBlock);
  cursor = end;
  touched++;
  console.log(`math-11-${id}: rewritten (${oldP3.length} -> ${newP3.length} chars)`);
}

parts.push(s.slice(cursor));
const out = parts.join("");
fs.writeFileSync(PATH, out);
console.log(`\nTouched ${touched} tasks. Wrote ${PATH}`);

// verify 1 and 52
for (const id of [1, 52]) {
  // re-parse
  const s2 = fs.readFileSync(PATH, "utf8");
  const re2 = /id: `math-11-(\d+)`/g;
  const ids2 = [];
  let m2;
  while ((m2 = re2.exec(s2))) ids2.push({ id: +m2[1], idx: m2.index });
  const i = ids2.findIndex((x) => x.id === id);
  const start = ids2[i].idx;
  const end = i + 1 < ids2.length ? ids2[i + 1].idx : s2.length;
  const block = s2.slice(start, end);
  const p = block.indexOf("**Part 3: Solve.**");
  const a = block.indexOf("**Answer.**");
  console.log(`\n===== VERIFY math-11-${id} =====`);
  console.log(block.slice(p, a));
}
