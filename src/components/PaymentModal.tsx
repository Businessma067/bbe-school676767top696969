...
import { getMyDiscountClaim, validateDiscountCode } from "@/lib/promo.functions";
...
export function PaymentModal({
  ...
  const showDiscountedTotal = true;
  ...
  const handleApplyDiscountOnCard = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!handled) {
        setError("This promocode is invalid or has already been used.");
      }
    } catch (err) {
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Complete your one-time purchase of {productName}. Apply a 15% promocode if you have one.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="mt-2 space-y-4">
            <p className="text-sm leading-relaxed text-muted-foreground">
              Pay securely by card through Monobank. You will be taken to the bank&apos;s checkout page
              and returned here right after the payment.
            </p>

            <form onSubmit={handleApplyDiscountOnCard} className="space-y-2">
              <label className="block text-xs font-medium text-foreground">Discount promocode</label>
            ...
            {discountApplied && (
              ...
              >
                {discountPct}% off locked on this account ({appliedPromoCode}) — forever €{eurPriceLabel}{" "}
                instead of €{catalogPriceLabel}
              </p>
            )}
