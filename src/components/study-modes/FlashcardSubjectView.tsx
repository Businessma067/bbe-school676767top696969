...
export type FlashcardSubjectViewModel = {
  id: string;
  title: string;
  description: string;
  accent: string;
...
type FlashcardSubjectViewProps = {
  subjectId: string;
  subject: FlashcardSubjectViewModel;
  /** localStorage / progress key — defaults to subjectId */
...
export function FlashcardSubjectView({
  ...
  const [deck, setDeck] = useState<DeckCard[]>(() => {
    ...
    return demoRevealLocked ? built.slice(0, 1) : built;
  });
  ...
  useEffect(() => {
    ...
    let nextIndex = 0;
    if (!demoRevealLocked && vocabularyModes && prevTerm) {
  ...
  }, [sectionId, subject.sections, progressKey, vocabularyModes, demoRevealLocked]);
  ...
  /** Demo: any navigation / mix-up beyond the first sample opens the unlock lock. */
  ...
  const allKeys = useMemo(
    () => buildDeck(progressKey, subject.sections, "all").map((c) => c.key),
    [subject.sections, progressKey],
  );
  ...
  useEffect(() => {
    if (vocabularyModes && sectionId === "all") {
  ...
  const rateCard = useCallback(
    (status: CardKnowledge) => {
      ...
      void upsertFlashcardProgress(progressKey, key, status);
      ...
      <main className="overflow-x-clip px-3 py-6 sm:px-6 sm:py-10 lg:px-8">
        <div className="mx-auto max-w-3xl">
          <div className="mb-8 flex flex-wrap items-start justify-between gap-4">
            ...
            {subject.downloadHref && !demoRevealLocked && (
              <a
          ...
          <div className="mb-6 flex flex-wrap items-center gap-3">
            ...
            <button
              ...
              onClick={() => {
                ...
                progressRef.current = {};
                queueRef.current = [];
                setDeck(buildDeck(progressKey, subject.sections, sectionId));
                setIndex(0);
                setFlipped(false);
                setSeen(0);
              }}
