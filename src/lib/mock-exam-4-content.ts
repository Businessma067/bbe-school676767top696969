/**
 * Curated Mock Exam 4 content (specified bank + generated tasks):
 * - Economics → English (Doomsday Glacier) → Math, in that order
 * - Economics: chapter bank cases + generated working-capital / ROCE chart case
 * - English: T.12 reading/vocab with passage, then grammar/vocab without + G.12.19
 * - Math: diversified MOCK engines + unused bank picks; EUR / USD outside KaTeX
 */
...
function unwrapSourced(mod: unknown): SourcedBundle {
  ...
  if (
    ...
    throw new Error("Mock Exam 4 sourced bank failed to load.");
  }
  return candidate;
}
...
export const MOCK_EXAM_4_ENGLISH_POINTS = [
...
export const MOCK_EXAM_4_MATH_POINTS = MATH_POINTS_PER_TASK.slice(0, 13);

export const MOCK_EXAM_4_QUESTION_COUNT =
  bundle.economics.length + bundle.english.tasks.length + bundle.math.length;

export const MOCK_EXAM_4_POINTS_TOTAL =
  bundle.economics.length * SCORING_CONFIG.economics.defaultMaxPerTask +
  MOCK_EXAM_4_ENGLISH_POINTS.reduce((a, b) => a + b, 0) +
  MOCK_EXAM_4_MATH_POINTS.reduce((a, b) => a + b, 0);
...
export const MOCK_EXAM_4_CONTENT_REV =
  "2026-09-15b · 34q · Glacier · teacher expls + katex scrub";
...
/** Build the full Mock Exam 4 question list (economics → english → math). */
export function buildMockExam4Questions(examId = "mock-4"): ExamQuestion[] {
  ...
  for (let i = 0; i < bundle.english.tasks.length; i++) {
    ...
    questions.push(
      fromBankTask({
        ...
        index,
        subject: "english",
        maxPoints: MOCK_EXAM_4_ENGLISH_POINTS[i] ?? ENGLISH_POINTS_BY_TYPE.vocabulary,
  ...
  for (let i = 0; i < bundle.math.length; i++) {
    ...
    questions.push(
      fromBankTask({
        ...
        index,
        subject: "math",
        maxPoints: MOCK_EXAM_4_MATH_POINTS[i] ?? 5,
        stem: (task.context ?? "").trim() || task.title || `Task ${index}`,
        statements: task.statements ?? [],
        answerKey: task.answer_key ?? [],
        explanations: task.tactical_explanations ?? [],
