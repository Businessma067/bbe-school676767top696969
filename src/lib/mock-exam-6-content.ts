...
function unwrapSourced(mod: unknown): SourcedBundle {
  ...
  if (
    ...
    throw new Error("Mock Exam 6 sourced bank failed to load.");
  }
  return candidate;
}
...
export const MOCK_EXAM_6_ENGLISH_POINTS = [
...
export const MOCK_EXAM_6_MATH_POINTS = MATH_POINTS_PER_TASK.slice(0, 13);

export const MOCK_EXAM_6_QUESTION_COUNT =
  bundle.economics.length + bundle.english.tasks.length + bundle.math.length;

export const MOCK_EXAM_6_POINTS_TOTAL =
  bundle.economics.length * SCORING_CONFIG.economics.defaultMaxPerTask +
  MOCK_EXAM_6_ENGLISH_POINTS.reduce((a, b) => a + b, 0) +
  MOCK_EXAM_6_MATH_POINTS.reduce((a, b) => a + b, 0);
...
export const MOCK_EXAM_6_CONTENT_REV =
  "2026-09-18i · 34q · Reshoring · natural math explanations + UI clip fix";
...
/** Build the full Mock Exam 6 question list (economics → english → math). */
export function buildMockExam6Questions(examId = "mock-6"): ExamQuestion[] {
  ...
  for (let i = 0; i < bundle.english.tasks.length; i++) {
    ...
    questions.push(
      fromBankTask({
        ...
        index,
        subject: "english",
        maxPoints: MOCK_EXAM_6_ENGLISH_POINTS[i] ?? ENGLISH_POINTS_BY_TYPE.vocabulary,
  ...
  for (let i = 0; i < bundle.math.length; i++) {
    ...
    questions.push(
      fromBankTask({
        ...
        index,
        subject: "math",
        maxPoints: MOCK_EXAM_6_MATH_POINTS[i] ?? 5,
        stem: (task.context ?? "").trim() || task.title || `Task ${index}`,
        statements: task.statements ?? [],
        answerKey: task.answer_key ?? [],
        explanations: task.tactical_explanations ?? [],
