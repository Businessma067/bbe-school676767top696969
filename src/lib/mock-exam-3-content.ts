/**
 * Curated Mock Exam 3 content (specified bank + generated tasks):
 ...
 * - Math: specified IDs; ch5 mixture systems and ch10 Newton-cooling + deposit generated
 */
...
function unwrapSourced(mod: unknown): SourcedBundle {
  ...
  if (
    ...
    throw new Error("Mock Exam 3 sourced bank failed to load.");
  }
  return candidate;
}
...
export const MOCK_EXAM_3_ENGLISH_POINTS = [
...
export const MOCK_EXAM_3_MATH_POINTS = MATH_POINTS_PER_TASK.slice(0, 13);

export const MOCK_EXAM_3_QUESTION_COUNT =
  bundle.economics.length + bundle.english.tasks.length + bundle.math.length;

export const MOCK_EXAM_3_POINTS_TOTAL =
  bundle.economics.length * SCORING_CONFIG.economics.defaultMaxPerTask +
  MOCK_EXAM_3_ENGLISH_POINTS.reduce((a, b) => a + b, 0) +
  MOCK_EXAM_3_MATH_POINTS.reduce((a, b) => a + b, 0);
...
export const MOCK_EXAM_3_CONTENT_REV =
  "2026-09-15a · 34q · Silicon Chokepoint · rights+real · mix/cool/Bayes";
...
/** Build the full Mock Exam 3 question list (economics → english → math). */
export function buildMockExam3Questions(examId = "mock-3"): ExamQuestion[] {
  ...
  for (let i = 0; i < bundle.english.tasks.length; i++) {
    ...
    questions.push(
      fromBankTask({
        ...
        index,
        subject: "english",
        maxPoints: MOCK_EXAM_3_ENGLISH_POINTS[i] ?? ENGLISH_POINTS_BY_TYPE.vocabulary,
        ...
        // Silicon Chokepoint passage on T.15.01–05 and T.15.08–09.
  ...
  for (let i = 0; i < bundle.math.length; i++) {
    ...
    questions.push(
      fromBankTask({
        ...
        index,
        subject: "math",
        maxPoints: MOCK_EXAM_3_MATH_POINTS[i] ?? 5,
        stem: (task.context ?? "").trim() || task.title || `Task ${index}`,
        statements: task.statements ?? [],
        answerKey: task.answer_key ?? [],
        explanations: task.tactical_explanations ?? [],
