...
function unwrapSourced(mod: unknown): SourcedBundle {
  ...
  if (
    ...
    throw new Error("BBE Demo Mock sourced bank failed to load.");
  }
  return candidate;
}
...
export const MOCK_EXAM_DEMO_ENGLISH_POINTS = [
...
export const MOCK_EXAM_DEMO_POINTS_TOTAL =
  ...
  MOCK_EXAM_DEMO_ENGLISH_POINTS.reduce((a, b) => a + b, 0) +
  MOCK_EXAM_DEMO_MATH_POINTS.reduce((a, b) => a + b, 0);

export const MOCK_EXAM_DEMO_CONTENT_REV =
  "2026-09-24c · 34q · free demo · hardest course econ/english/math · auth gate";
...
/** Build the free BBE Demo Mock question list (economics → english → math). */
export function buildMockExamDemoQuestions(examId = "demo-mock"): ExamQuestion[] {
  ...
  for (let i = 0; i < bundle.english.tasks.length; i++) {
    ...
    questions.push(
      fromBankTask({
        ...
        index,
        subject: "english",
        maxPoints: MOCK_EXAM_DEMO_ENGLISH_POINTS[i] ?? ENGLISH_POINTS_BY_TYPE.vocabulary,
  ...
  for (let i = 0; i < bundle.math.length; i++) {
    ...
    questions.push(
      fromBankTask({
        ...
        index,
        subject: "math",
        maxPoints: MOCK_EXAM_DEMO_MATH_POINTS[i] ?? 5,
        stem: (task.context ?? "").trim() || task.title || `Task ${index}`,
        statements: task.statements ?? [],
        answerKey: task.answer_key ?? [],
        explanations: task.tactical_explanations ?? [],
