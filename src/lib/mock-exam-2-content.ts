/**
 * Curated Mock Exam 2 content (specified bank + generated tasks):
 * - Economics → English (Shifting Anatomy) → Math, in that order
 * - Economics: chapter bank cases + generated share/bond/rates chart case
 * - English: T.15 reading/vocab with passage, then grammar/vocab without + G.6.19
 * - Math: specified IDs; ch6 inequalities and ch10 radioactive exp/log generated
 */

import sourced from "@/data/mock-exam-2-sourced.json";
import {
...
function unwrapSourced(mod: unknown): SourcedBundle {
  ...
  if (
    ...
    throw new Error("Mock Exam 2 sourced bank failed to load.");
  }
  return candidate;
}
...
export const MOCK_EXAM_2_ENGLISH_POINTS = [
...
export const MOCK_EXAM_2_MATH_POINTS = MATH_POINTS_PER_TASK.slice(0, 13);

export const MOCK_EXAM_2_QUESTION_COUNT =
  bundle.economics.length + bundle.english.tasks.length + bundle.math.length;

export const MOCK_EXAM_2_POINTS_TOTAL =
  bundle.economics.length * SCORING_CONFIG.economics.defaultMaxPerTask +
  MOCK_EXAM_2_ENGLISH_POINTS.reduce((a, b) => a + b, 0) +
  MOCK_EXAM_2_MATH_POINTS.reduce((a, b) => a + b, 0);

/** Bump when Mock 2 bank content changes — shown in UI so Lovable preview sync can be verified. */
export const MOCK_EXAM_2_CONTENT_REV =
  "2026-09-14a · 34q · Shifting Anatomy · stock+bonds · hard ineq/exp";
...
/** Build the full Mock Exam 2 question list (economics → english → math). */
export function buildMockExam2Questions(examId = "mock-2"): ExamQuestion[] {
  ...
  for (let i = 0; i < bundle.english.tasks.length; i++) {
    ...
    questions.push(
      fromBankTask({
        ...
        index,
        subject: "english",
        maxPoints: MOCK_EXAM_2_ENGLISH_POINTS[i] ?? ENGLISH_POINTS_BY_TYPE.vocabulary,
        ...
        // Shifting Anatomy passage on T.15.01–05 and T.15.08–09.
  ...
  for (let i = 0; i < bundle.math.length; i++) {
    ...
    questions.push(
      fromBankTask({
        ...
        index,
        subject: "math",
        maxPoints: MOCK_EXAM_2_MATH_POINTS[i] ?? 5,
        stem: (task.context ?? "").trim() || task.title || `Task ${index}`,
        statements: task.statements ?? [],
        answerKey: task.answer_key ?? [],
        explanations: task.tactical_explanations ?? [],
