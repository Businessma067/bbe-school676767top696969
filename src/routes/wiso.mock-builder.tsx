...
import {
  ...
  getEnabledWisoCustomMockSubjects,
  maxQuestionsForWisoChapters,
  wisoSubjectFromDb,
  type WisoCustomMockSubjectId,
} from "@/config/wiso-custom-mock-builder";
import { displayTitleForCustomMock } from "@/config/custom-mock-builder";
import {
  cacheCustomMock,
  fetchCustomMockById,
  fetchWisoCustomMocks,
  type WisoGenerateArgs,
} from "@/lib/custom-mock-builder/client";
import { buildWisoCustomMock } from "@/lib/wiso-custom-mock-builder/build.functions";
...
function historySubjectMeta(dbSubject: string): {
  label: string;
  badgeClass: string;
} {
...
export function WisoMockBuilderPage() {
  const navigate = useNavigate();
  const buildFn = useServerFn(buildWisoCustomMock);
  const subjects = useMemo(() => getEnabledWisoCustomMockSubjects(), []);

  const [authReady, setAuthReady] = useState(false);
  const [subject, setSubject] = useState<WisoCustomMockSubjectId>("economics");
  ...
  const bookChapters = useMemo(() => getWisoCustomMockChapters(subject), [subject]);
  const subjectCfg = WISO_CUSTOM_MOCK_SUBJECTS[subject];
  ...
  useEffect(() => {
    ...
    (async () => {
      setAuthReady(true);
      const list = await fetchWisoCustomMocks();
      if (cancelled) return;
      setHistory(list);
  ...
  const maxQuestions = useMemo(() => {
    const caps = wisoChaptersFromSubtopicIds(subject, selectedSubtopics);
    return Math.min(maxQuestionsForWisoChapters(caps), CUSTOM_MOCK_MAX_QUESTIONS);
  ...
  const weightTopics: TopicWeightTopic[] = useMemo(() => {
    return [...selectedSubtopics]
      ...
      .map((id) => {
        const meta = findWisoCustomMockSubtopic(subject, id);
        return {
          id,
  ...
  const onBuild = async () => {
    ...
    try {
      ...
      const summary: CustomMockSummary = {
        ...
        title: result.title,
        subject: row?.subject ?? WISO_CUSTOM_MOCK_SUBJECTS[subject].dbSubject,
      ...
      const raw = e instanceof Error ? e.message : "Mock konnte nicht erstellt werden. Bitte erneut versuchen.";
      setError(/unauthorized/i.test(raw) ? "Bitte erneut anmelden, um einen Mock zu erstellen." : raw);
    } finally {
      setBuilding(false);
    }
  };
        ...
        <div className="mx-auto max-w-6xl">
          <div className="mb-10 text-center">
            ...
            <p className="mx-auto mt-4 max-w-xl text-lg text-muted-foreground">
              ...
              Full Course erstellen. Bis zu {CUSTOM_MOCK_MAX_QUESTIONS} Fragen.
            </p>
          ...
          >
            <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.15fr)] lg:items-start">
              <div className="min-w-0">
                <h2 className="font-display text-xl font-semibold">Themen & Unterkapitel</h2>
                <p className="mt-1 text-sm text-muted-foreground">
                  Kapitel öffnen und Abschnitte anhaken. Gewählte Themen erscheinen rechts als
                  Gewichte.
                </p>
                          ...
                          >
                            ...
                            {selectedInCh > 0 && (
                              <span
                                ...
                                style={{
                                  backgroundColor: `color-mix(in oklab, ${accent} 18%, var(--card))`,
                                  color: accent,
                                }}
                              >
                ...
                <div className="mt-3 flex flex-wrap gap-4 text-xs text-muted-foreground">
                  ...
                  {selectedSubtopics.length > 0 && (
                    <span>
                      {selectedSubtopics.length} Unterkapitel
                    </span>
                  )}
                </div>
              </div>
            ...
            >
              {building ? (
                ...
                <>
                  ...
                  {subjectCfg.label}-Mock aus dem Full Course erstellen
                </>
              )}
            </button>
          </section>
          ...
          <section className="mt-12">
            <h2 className="mb-5 font-display text-xl font-semibold">Deine WiSo Custom Mocks</h2>
            {history === null ? (
              <p className="text-sm text-muted-foreground">Laden…</p>
            ) : history.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-border bg-card/50 p-10 text-center text-sm text-muted-foreground">
                Noch keine Custom Mocks. Fach wählen, Unterkapitel anhaken und oben den Mix
                festlegen.
              </div>
            ) : (
              <div className="grid gap-4">
                {history.map((mock) => {
                  const meta = historySubjectMeta(mock.subject);
                  return (
                    <div
                      key={mock.id}
                    ...
                    >
                      <div>
                        ...
                        <p className="mt-1 text-xs text-muted-foreground">
                          {mock.questionCount} Fragen · {mock.durationMinutes} Min. ·{" "}
