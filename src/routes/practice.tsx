...
export const Route = createFileRoute("/practice")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Practice true/false questions with instant feedback and explanations.",
      },
                  ...
                  <li key={s.id} className="rounded-lg border border-border p-4 space-y-3">
                    <div className="flex items-start gap-3">
                      ...
                      {submitted && (
                        <span
                          className={
                            "text-lg font-bold " + (isCorrect ? "text-green-600" : "text-red-600")
                          }
                      ...
                      {([true, false] as const).map((val) => {
                        ...
                        return (
                          <button
                            ...
                            type="button"
                            disabled={submitted}
                            onClick={() => setAnswers((a) => ({ ...a, [s.id]: val }))}
                            className={
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      const correctCount = statements.filter(
                        (s) => answers[s.id] === s.correct_answer,
                      ).length;
                      const subjectId =
                        question.subject_id ?? question.topics?.subjects?.id ?? null;
