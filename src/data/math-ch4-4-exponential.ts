...
export const MATH_CH4_4_EXPONENTIAL: MathTask[] = [
  {
    ...
    statements: [
      `Matching bases in $2^x = 32$ determines a unique real exponent because base $2$ is strictly increasing.`,
      `No real value of $x$ makes $2^x$ equal $-8$.`,
      `A domain check shows $\\log_3 x = 2$ admits only one positive root.`,
      `A student claims that $5^x = -1$ can hold for some real $x$.`,
      `Rewriting $\\left(\\frac{1}{2}\\right)^x = \\frac{1}{8}$ as $2^{-x} = 2^{-3}$ leaves a unique exponent.`,
    ],
...
Decay base $\\frac{1}{2}$ is still one-to-one on $\\mathbb{R}$, so exactly one real exponent works.`,
  ...
  },
  {
    ...
    context: `Evaluate each statement. Mark it TRUE or FALSE.`,
    statements: [
      `After writing $1000 = 10^3$, the equation $10^{x-1} = 1000$ becomes linear in the exponent.`,
      `Expressing $4^x = 2$ on base $2$ gives $2^{2x} = 2$, reducing to a linear equation in the exponent.`,
      `A candidate reports that $7^x = 0$ is satisfied when $x = 0$.`,
      `The logarithmic definition turns $\\log_5 125 = x$ into $5^x = 125$.`,
      `Strict monotonicity of base $3$ in $3^{2x} = 81$ rules out multiple distinct exponents.`,
    ],
...
Strict monotonicity gives exactly one real solution, not more than one.`,
  ...
  },
  {
    ...
    context: `Evaluate each statement. Mark it TRUE or FALSE.`,
    statements: [
      `Matching bases in $4^{2x} = 8^{x+1}$ leaves a linear equation in the exponent.`,
      `Writing $9^x = 27$ as $3^{2x} = 3^3$ leaves a single rational root on the domain.`,
      `A student claims that $16^x = 4$ cannot be satisfied by any real exponent.`,
      `Every admissible root of $\\log_2 x = -3$ satisfies $0 < x < 1$.`,
      `A candidate reports that $2^{3x} = 4^{x+1}$ admits more than one real crossing.`,
    ],
    answer_key: [true, true, false, true, false],
    tactical_explanations: [
      `**A.** → True
...
Strict monotonicity yields exactly one real root, not more than one.`,
  ...
  },
  {
    ...
    context: `Evaluate each statement. Mark it TRUE or FALSE.`,
    statements: [
      `Common-base rewriting of $2^{x+1} = 8^{x-1}$ on base $2$ gives $x + 1 = 3x - 3$.`,
      `Since $81 = 3^4$, the equation $\\left(\\frac{1}{3}\\right)^x = 81$ forces $-x = 4$.`,
      `A student claims that $\\log_2 x = 5$ is satisfied by a negative value of $x$.`,
      `Every admissible root of $6^x = 216$ satisfies $x > 0$.`,
      `A student claims that $3^{2x} = 81$ has no admissible real exponent.`,
    ],
    answer_key: [true, true, false, true, false],
    tactical_explanations: [
      `**A.** → True
...
A real solution exists, contradicting the claim of no real solution.`,
  ...
  },
  {
    ...
    context: `Evaluate each statement. Mark it TRUE or FALSE.`,
    statements: [
      `Rewriting $25^x = 125$ on base $5$ equates the exponents $2x$ and $3$.`,
      `Matching bases in $2^{2x+1} = 32$ gives $2x + 1 = 5$.`,
      `A student claims that every root of $5^{x-2} = \\frac{1}{25}$ is negative.`,
      `On domain $x > 0$, $\\log_{10} x = -1$ admits exactly one positive admissible root.`,
      `A candidate reports that $3^x = \\frac{1}{9}$ is satisfied by a positive exponent.`,
    ],
    answer_key: [true, true, false, true, false],
    tactical_explanations: [
      `**A.** → True
...
The unique real solution is negative, not positive.`,
  ...
  },
  {
    ...
    context: `Evaluate each statement. Mark it TRUE or FALSE.`,
    statements: [
      `Strict monotonicity of $e^x$ forces a unique preimage for $e^x = 20$.`,
      `No real value of $x$ makes $e^x$ negative, so $e^x = -1$ is impossible.`,
      `A student claims that $e^{2x} = e^6$ yields more than one distinct real exponent.`,
      `Every admissible root of $\\ln x = 1$ satisfies $x > 0$.`,
      `A candidate reports that $e^x = 0$ is satisfied when $x = -1$.`,
    ],
    answer_key: [true, true, false, true, false],
    tactical_explanations: [
      `**A.** → True
...
There are zero real solutions, not exactly one.`,
  ...
  },
  {
    ...
    context: `Evaluate each statement. Mark it TRUE or FALSE.`,
    statements: [
      `The substitution $u = 2x - 1$ turns $e^{2x-1} = 5$ into $e^u = 5$ with $5 > 0$.`,
      `Dividing $2e^x = 10$ by $2$ isolates $e^x = 5$, which has a unique preimage.`,
      `Equating exponents in $e^{x+2} = e^{x-1}$ leads to the contradiction $2 = -1$.`,
      `The rule $\\ln(2x) = 0$ with $2x > 0$ admits only one positive admissible root.`,
      `A student claims that $e^{-x} = e^x$ cannot hold for any real $x$.`,
    ],
...
There is exactly one real solution, so the claim of no real solution is false.`,
  ...
  },
  {
    ...
    context: `Evaluate each statement. Mark it TRUE or FALSE.`,
    statements: [
      `On domain $x > 0$, $\\log_2 x = 5$ determines a unique positive admissible root.`,
      `A student claims that $\\log_3 x = 2$ is satisfied by a negative value of $x$.`,
      `A domain check on $\\log_3(x-1) = 2$ shows the unique admissible root exceeds $1$.`,
      `With $1 - x > 0$, the equation $\\ln(1-x) = 0$ leaves a single admissible root on the domain.`,
      `Strict monotonicity of $\\ln$ on $(0,\\infty)$ means $\\ln x = 0$ cannot hold at two distinct positive values.`,
    ],
...
Only one admissible real value exists.`,
  ...
  },
  {
    ...
    context: `Evaluate each statement. Mark it TRUE or FALSE.`,
    statements: [
      `With $x + 1 > 0$, the equation $\\ln(x+1) = 2$ determines a unique admissible root.`,
      `Domain $2x - 1 > 0$ is required before solving $\\ln(2x-1) = 3$.`,
      `Equal natural logs in $\\ln(5-x) = \\ln 2$ force $5 - x = 2$ with $5 - x > 0$.`,
      `A candidate reports that $\\log_2(x-3) = 1$ is satisfied by some $x \\le 3$.`,
      `Every admissible root of $\\ln x + \\ln(x-1) = \\ln 12$ satisfies $x > 1$.`,
    ],
...
Together these force $x > 1$ for any valid solution.`,
  ...
  },
  {
    ...
    context: `Evaluate each statement. Mark it TRUE or FALSE.`,
    statements: [
      `Using $\\log a + \\log b = \\log(ab)$ on $\\log x + \\log(x+3) = 1$ yields a quadratic; domain $x > 0$ keeps one positive root.`,
      `A student claims that $\\log(x+2) + \\log(x-1) = 1$ is satisfied by some $x \\le 1$.`,
      `The equation $\\ln(x^2) = 0$ gives $x^2 = 1$, producing two symmetric roots after the domain check.`,
      `Translating $\\log_{10}(x-9) = 1$ gives $x - 9 = 10$ with $x - 9 > 0$.`,
      `A candidate reports that $\\log_2(x-4) = 3$ is satisfied by some $x \\le 4$.`,
    ],
    answer_key: [true, false, true, true, false],
    tactical_explanations: [
      `**A.** → True
...
The admissible root is $x = 12$, which is not $\\le 4$.`,
  ...
  },
  {
    ...
    context: `Evaluate each statement. Mark it TRUE or FALSE.`,
    statements: [
      `Converting $\\log_5 x = 2$ to exponential form with $x > 0$ admits only one positive admissible root.`,
      `The equation $\\log_2(x^2 - 1) = 3$ with $|x| > 1$ produces two symmetric roots after the domain check.`,
      `A student claims that $\\log_{10} x = 2$ is satisfied by some $x < 1$.`,
      `Squaring $\\ln(x^2) = 2$ on domain $x^2 > 0$ yields two symmetric admissible roots.`,
      `A candidate reports that $\\log_2 x = \\log_2 8$ holds for more than one positive value of $x$.`,
    ],
    answer_key: [true, true, false, true, false],
    tactical_explanations: [
      `**A.** → True
...
Domain $x > 0$ is satisfied, and the solution is unique.`,
  ...
  },
  {
    ...
    context: `Evaluate each statement. Mark it TRUE or FALSE.`,
    statements: [
      `Change of base identifies $\\dfrac{\\log_2 x}{\\log_2 4} = 3$ with $\\log_4 x = 3$.`,
      `Every admissible root of $\\log_3 x = 2$ satisfies $x \\le 10$, but none satisfies $x > 10$.`,
      `On domain $x > 0$, $\\log_4 x = 2$ determines a unique positive admissible root.`,
      `Since $\\ln$ and $e^x$ are inverses, $\\ln(e^x) = 2$ reduces to a linear equation in the exponent.`,
      `A student claims that $\\log_{10}(0.01) = x$ is satisfied by a positive exponent.`,
    ],
    answer_key: [true, true, true, true, false],
    tactical_explanations: [
      `**A.** → True
...
The unique real solution is negative, not positive.`,
  ...
  },
  {
    ...
    context: `Evaluate each statement. Mark it TRUE or FALSE.`,
    statements: [
      `Evaluating $\\log_8 2 = \\frac{1}{3}$ turns $\\log_2 x = \\log_8 2$ into an exponential form with a unique positive admissible root.`,
      `Strict monotonicity of $\\log_{10}$ on $(0,\\infty)$ gives a unique preimage for $\\log_{10} x = 2$.`,
      `Converting $\\log_9 x = \\frac{1}{2}$ admits only one positive admissible root on domain $x > 0$.`,
      `A candidate reports that $\\log_2 x = 3$ is satisfied by some $x < 1$.`,
      `The product rule on $\\log x + \\log 2 = 1$ yields $\\log(2x) = 1$ with $x > 0$.`,
    ],
    answer_key: [true, true, true, false, true],
    tactical_explanations: [
      `**A.** → True
...
Exactly one positive admissible root exists.`,
  ...
  },
  {
    ...
    context: `Evaluate each statement. Mark it TRUE or FALSE.`,
    statements: [
      `The argument $x - 3$ must be positive before $\\log_2(x-3) = 1$ is defined, so no $x \\le 3$ works.`,
      `Every admissible root of $\\ln(5-x) = \\ln 2$ satisfies $x < 5$.`,
      `A student claims that every root of $\\log(x^2) = 1$ is positive.`,
      `Both factors in $\\ln x + \\ln(x-1) = \\ln 12$ require $x > 1$.`,
      `The argument $x - 4$ must be positive before $\\log_2(x-4) = 3$ is defined, so no $x \\le 4$ works.`,
    ],
...
No value with $x \\le 4$ can satisfy the equation.`,
  ...
  },
  {
    ...
    context: `Evaluate each statement. Mark it TRUE or FALSE.`,
    statements: [
      `The rule $\\ln u = 0$ with $u = 2x - 5 > 0$ leaves a single admissible root on the domain.`,
      `Squaring $\\sqrt{\\ln(x+4)} = 2$ requires $x + 4 > 0$ and $\\ln(x+4) \\ge 0$.`,
      `On domain $x > 0$, $(\\ln x)^2 = 4$ gives $\\ln x = \\pm 2$, both yielding positive roots.`,
      `Adding logarithms in $\\ln x = -\\ln x$ on $x > 0$ forces $\\ln x = 0$.`,
      `A student claims that $\\ln(2x-5) = 0$ is satisfied by some $x \\le 2$.`,
    ],
    answer_key: [true, true, true, true, false],
    tactical_explanations: [
      `**A.** → True
...
The admissible root exceeds $2$, so no value with $x \\le 2$ works.`,
  ...
  },
  {
    ...
    context: `Evaluate each statement. Mark it TRUE or FALSE.`,
    statements: [
      `Using the product rule on $\\log_2(x-1) + \\log_2(x-3) = 2$ with $x > 3$ filters the quadratic to one admissible root.`,
      `Substituting $u = e^x > 0$ in $e^{2x} - 5e^x + 6 = 0$ yields two positive values of $u$.`,
      `The equation $\\ln|x| = 0$ gives $|x| = 1$, producing two symmetric roots after the domain check.`,
      `With $|x| > 1$, the equation $\\log(x^2 - 1) = 1$ yields two symmetric admissible roots.`,
      `Dividing $2^x = 3^{x-1}$ by $2^x > 0$ gives $\\left(\\frac{3}{2}\\right)^x = 3$, which crosses once by monotonicity.`,
    ],
...
The function $\\left(\\frac{3}{2}\\right)^x$ is strictly increasing, so exactly one real crossing exists.`,
  ...
  },
  {
    ...
    context: `Evaluate each statement. Mark it TRUE or FALSE.`,
    statements: [
      `Substituting $u = e^x > 0$ in $e^{2x} - 7e^x + 12 = 0$ gives $u = 3$ or $u = 4$, each yielding one real $x$.`,
      `Letting $u = 2^x > 0$ turns $3 \\cdot 2^x + 2^{-x} = 5$ into a quadratic with two positive roots.`,
      `By definition, $\\log_x 8 = 3$ means $x^3 = 8$ with $x > 0$ and $x \\neq 1$.`,
      `Writing $\\log_4 x = \\frac{\\log_2 x}{2}$ combines $\\log_2 x + \\log_4 x = 3$ into one base-$2$ equation.`,
      `A student claims that $\\ln(x^2 - 1) = 1$ produces more than two real roots.`,
    ],
    answer_key: [true, true, true, true, false],
    tactical_explanations: [
      `**A.** → True
...
Both satisfy the domain, giving exactly two real solutions, not more than two.`,
  ...
  },
  {
    ...
    context: `Evaluate each statement. Mark it TRUE or FALSE.`,
    statements: [
      `Factoring $2^{x+1} - 2^x = 16$ as $2^x = 16$ uses strict monotonicity of base $2$.`,
      `Equal base-$3$ logs in $\\log_3(x+4) = \\log_3(2x-1)$ force $x + 4 = 2x - 1$ with both arguments positive.`,
      `Multiplying $e^x - 3e^{-x} = 2$ by $e^x > 0$ and substituting $u = e^x$ leaves only $u = 3$ admissible.`,
      `Rewriting $5^{2x} = 25^{x-1}$ on base $5$ gives $2x = 2x - 2$, which is impossible.`,
      `Equal natural logs in $\\ln(3x+1) = \\ln 7$ force $3x + 1 = 7$ with $3x + 1 > 0$.`,
    ],
    answer_key: [true, true, true, true, true],
    tactical_explanations: [
      `**A.** → True
...
Also $2 > -\\frac{1}{3}$, giving exactly one admissible root.`,
  ...
  },
  {
    ...
    context: `Evaluate each statement. Mark it TRUE or FALSE.`,
    statements: [
      `The product rule on $\\log_2(x+1) + \\log_2(x-1) = 3$ with $x > 1$ filters the quadratic to one admissible root.`,
      `Equal base-$10$ logs in $\\log(x+5) = \\log(2x)$ force $x + 5 = 2x$ with both arguments positive.`,
      `Matching bases in $4^x = 2^{x+3}$ gives $2x = x + 3$ on base $2$.`,
      `Equal natural logs in $\\ln(x-2) = \\ln(2x-5)$ require $x - 2 = 2x - 5$ with both arguments positive.`,
      `Dividing $10^x = 10^{-x}$ by $10^x > 0$ gives $10^{2x} = 1$, which determines a unique real exponent.`,
    ],
    answer_key: [true, true, true, true, true],
    tactical_explanations: [
      `**A.** → True
...
Exactly one real solution exists.`,
  ...
  },
  {
    ...
    context: `Evaluate each statement. Mark it TRUE or FALSE.`,
    statements: [
      `Dividing $2^x = 3^x$ by $2^x > 0$ gives $\\left(\\frac{3}{2}\\right)^x = 1$, which determines a unique real exponent.`,
      `A student claims that $\\left(\\frac{1}{2}\\right)^x = \\left(\\frac{1}{3}\\right)^x$ holds for more than one real exponent.`,
      `The inequality $e^x \\ge x + 1$ shows $e^x - x \\ge 1 > 0$ for every real $x$.`,
      `On $x > 0$, the function $f(x) = \\ln x + x - 1$ is strictly increasing with $f(1) = 0$ for $\\ln x = 1 - x$.`,
      `Squaring $\\sqrt{\\ln x} = 1$ on domain $x > 0$ and $\\ln x \\ge 0$ admits only one positive admissible root.`,
    ],
...
Exactly one admissible root exists.`,
  ...
  },
  {
    ...
    statements: [
      `The product rule on $\\log_2(x-1) + \\log_2(x+1) = 3$ with $x > 1$ filters the quadratic to one admissible root.`,
      `Dividing $e^{2x} = 4e^x$ by $e^x > 0$ isolates $e^x = 4$.`,
      `Dividing $2 \\cdot 3^x = 54$ by $2$ gives $3^x = 27$, which determines a unique real exponent.`,
      `A domain check on $\\log_3(2x-1) = 2$ shows the unique admissible root exceeds $\\frac{1}{2}$.`,
      `Equal natural logs in $\\ln(x^2 - 4) = \\ln 5$ give $x^2 = 9$ with $x^2 - 4 > 0$.`,
    ],
    answer_key: [true, true, true, true, true],
    tactical_explanations: [
      `**A.** → True
...
Both $\\pm \\sqrt{11}$ satisfy $|x| > 1$, giving exactly two real solutions.`,
  ...
  },
  {
    ...
    statements: [
      `Letting $u = 2^x > 0$ turns $3 \\cdot 2^x + 2^{-x} = 5$ into a quadratic with two positive roots.`,
      `With $x^2 - 1 > 0$, the equation $\\ln(x^2 - 1) = 1$ produces two symmetric roots after the domain check.`,
      `Arguments in $\\log_2(x-1) + \\log_2(x-3) = 2$ require $x > 3$, so no $x \\le 3$ is admissible.`,
      `Rewriting $5^{2x} = 25^{x-1}$ on base $5$ gives $2x = 2x - 2$, which is impossible.`,
      `A student claims that $e^x - 3e^{-x} = 2$ is satisfied by more than one real value of $x$.`,
    ],
    answer_key: [true, true, true, true, false],
    tactical_explanations: [
      `**A.** → True
...
Only $u = 3 > 0$ is valid, giving exactly one real solution, not more than one.`,
  ...
  },
  {
    ...
    statements: [
      ...
      `The equation $\\ln|x| = 0$ gives $|x| = 1$, producing two symmetric roots after the domain check.`,
      `Substituting $u = e^x > 0$ in $e^{2x} - 5e^x + 6 = 0$ yields two positive values of $u$.`,
      `Converting $\\log_9 x = \\frac{1}{2}$ admits only one positive admissible root on domain $x > 0$ with $x \\neq 1$.`,
      `A student claims that $\\log_2(x+1) + \\log_2(x-1) = 3$ with $x > 1$ yields two admissible roots.`,
    ],
    answer_key: [true, true, true, true, false],
    tactical_explanations: [
      `**A.** → True
...
Only $x = 3$ exceeds $1$, giving exactly one admissible root, not two.`,
  ...
  },
  {
    ...
    statements: [
      ...
      `Writing $\\log_9 x = \\frac{\\log_3 x}{2}$ combines $\\log_3 x + \\log_9 x = 2$ into one base-$3$ equation.`,
      `Letting $u = 2^x > 0$ turns $2 \\cdot 2^x + 2^{-x} = 3$ into a quadratic with two positive roots.`,
      `Rewriting $3^{x+1} = 9^{x-2}$ as $3^{x+1} = 3^{2x-4}$ equates linear exponents on base $3$.`,
      `Using $\\log a - \\log b = \\log(a/b)$ on $\\log(x+1) - \\log(x-1) = 1$ with $x > 1$ gives one admissible root.`,
    ],
    answer_key: [true, true, true, true, true],
    tactical_explanations: [
      `**A.** → True
...
Exactly one admissible root with $x > 1$ exists.`,
  ...
  },
  {
    ...
    context: `Evaluate each statement. Mark it TRUE or FALSE.`,
    statements: [
      `Substituting $u = e^x > 0$ in $e^{2x} - 9e^x + 20 = 0$ gives $u = 4$ or $u = 5$, each yielding one real $x$.`,
      `Taking natural logarithms of $5^x = 2^{x+1}$ yields one linear equation because the $x$-coefficients differ.`,
      `By definition, $\\log_x 16 = 2$ means $x^2 = 16$ with $x > 0$ and $x \\neq 1$.`,
      `Equal natural logs in $\\ln(x^2 - 4) = \\ln 5$ give $x^2 = 9$ with $x^2 - 4 > 0$.`,
      `Writing $\\log_8 x = \\frac{\\log_2 x}{3}$ combines $\\log_2 x + \\log_8 x = \\frac{4}{3}$ into one base-$2$ equation.`,
    ],
    answer_key: [true, true, true, true, true],
    tactical_explanations: [
      `**A.** → True
