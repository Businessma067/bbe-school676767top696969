...
**Exam note.** Across recent BBE years, financial mathematics on the exam has tended to use simpler setups: simple interest, basic compound growth, ordinary present values, and the more standard versions of the formulas in this chapter. The harder constructions below are still worth understanding, but the better exam strategy is to sharpen the core skills first. Master timing, rate conversion, discounting, ordinary annuities, and the plain loan and IRR patterns before trying to cover every advanced variant in the chapter.
