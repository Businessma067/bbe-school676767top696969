...
def process_explanation(
    ...
    # Targeted complete-square rewrite when compressed
    ...
    if cs is not None and ("complet" in stmt.lower() or "complet" in text[:80].lower()):
        # only replace if current is thin
        if text.count("$$") // 2 <= 6:
            text = cs
        ...
        if start <= ti < end_i:
            ...
            for i, (s, e, body) in enumerate(items):
                ...
                if re.match(r"\*\*[A-E]\.\*\*", body.lstrip()):
                    new = process_explanation(body, letter, truth, stmt, context)
                    # escape backticks for TS template literal
                    new_esc = new.replace("\\", "\\\\").replace("`", "\\`")
                    # but existing file uses \\ for latex — body already has \\ 
                    # The body from extract has actual backslashes as in file (\\)
                    # When we process, we work on decoded content as stored (with \\)
                    # Re-read: extract keeps \\ as two chars. process works on that.
                    # For write-back we need to wrap in `...` with same escaping as original.
                    # Original uses \\ for latex; backticks in math rare.
                    new_esc = new.replace("`", "\\`")
                    if new != body:
                        changed += 1
                        task_changed = True
                    problems.extend(audit_explanation(new, letter, truth, case_id))
