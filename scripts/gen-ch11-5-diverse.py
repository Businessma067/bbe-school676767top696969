    ...
    tasks.append(
        make_task(
            ...
            [
                ...
                r"Because $Q'(a)$ is decreasing, profit cannot have an interior maximum.",
            ],
...
Setting $\pi'=0$ gives $\sqrt{a}=50$, so $a=2500$. Also $\pi''<0$ there""",
                r"""Marginal revenue of advertising is $R'(a)=5Q'(a)=\dfrac{50}{\sqrt{a}}$. At $a=2500$ this equals $1$, matching the marginal cost of one more euro spent""",
                ...
                r"""At $a=100$, $\pi'(100)=\dfrac{50}{10}-1=4>0$, so a little more advertising still raises profit""",
                r"""A decreasing marginal product does not forbid an interior profit max; it is exactly what makes $\pi'$ cross from $+$ to $-$ once""",
            ],
            r"""Chain-rule revenue $R=100\sqrt{a}$ against linear advertising cost peaks at $a=2500$, where $R'=1$. Square-root response is concave, so doubling spend does not double listeners. Positive $\pi'$ at small $a$ means advertising is still worthwhile there.""",
        )
    )
    ...
    tasks.append(
        make_task(
            ...
            [
                ...
                r"""$A'(x)=120-4x=0$ at $x=30$. Also $A''=-4<0$, so this is a maximum""",
                r"""$y=120-2\cdot30=60$""",
                r"""$A''(30)=-4<0$, so the second-derivative test confirms a strict local maximum""",
                r"""Just left of $x=30$, $A'>0$, so decreasing the width a little lowers area rather than raising it""",
            ],
            r"""Reduce to one variable with the fencing constraint, maximise $A=120x-2x^{2}$ at $x=30$, $y=60$. The second derivative is negative, and the sign of $A'$ shows area still rises as one approaches $30$ from the left.""",
        )
    )
...
Which of the following statements is/are correct?""",
            ...
            [
                r"""$TC'(Q)=-\dfrac{20000}{Q^{2}}+2=0$ gives $Q^{2}=10000$, so $Q=100$""",
                r"""The two terms in $TC'$ balance when $\dfrac{20000}{Q^{2}}=2$; that is the EOQ first-order condition equating the magnitudes of the two marginal cost pieces""",
    ...
    tasks.append(
        make_task(
            ...
            [
                ...
                r"The founder is risk-averse on this interval because $U''(w)<0$.",
            ...
            [True, True, False, True, False],
            [
                r"""$U'(w)=20-0.1w>0$ on $(0,200)$ because $0.1w<20$ there""",
                r"""$U''(w)=-0.1<0$, so $U$ is strictly concave — the usual calculus signature of risk aversion""",
                r"""$U'=0$ at $w=200$, which is the right endpoint of the open interval; on $(0,200)$ utility is still strictly increasing, so there is no interior maximum below $200$""",
                r"""$U'(50)=20-5=15$""",
                r"""For a strictly concave $U$, Jensen's inequality goes the other way: a mean-preserving spread lowers expected utility""",
            ],
            r"""Positive but decreasing marginal utility ($U'>0$, $U''<0$) means risk aversion. Utility keeps rising on $(0,200)$. A mean-preserving spread reduces expected utility under concavity.""",
        )
    )
...
Change claim to y/x = p_y/p_x (wrong) to make false.""",
    ...
    tasks.append(
        make_task(
            ...
            r"""A bike workshop's weekly profit $P(Q)$ is not printed as a formula. Instead the figure shows marginal profit $P'(Q)$. Zeros are marked at $Q=2$ and $Q=8$. Use only the figure (and the usual sign reading of $P'$). Which of the following statements is/are correct?""",
            [
                ...
                r"At $Q=5$, reading the scale, $P'$ is greater than $6$.",
                r"Because $P'$ peaks somewhere near $Q=5$, that output maximises profit itself.",
            ],
            [True, True, True, True, False],
            [
                r"""Between the marked zeros the curve sits above the axis, so $P'>0$ on $(2,8)$: a little more output raises profit""",
                r"""At $Q=8$, $P'$ changes from $+$ to $-$, which is a local maximum of $P$""",
                r"""At $Q=2$, $P'$ changes from $-$ to $+$, which is a local minimum of $P$""",
                r"""The midpoint peak of this downward-opening parabola has height $-(5-2)(5-8)=9>6$""",
                r"""A peak of $P'$ is where profit climbs fastest, not where profit is largest. Profit peaks where $P'$ crosses down through zero""",
            ],
            r"""Read signs and crossings of $P'$ only. Expand on $(2,8)$, local min at $2$, local max at $8$. The peak of $P'$ is steepest climb, not the profit maximum.""",
            figure=fig22,
        )
    )
    ...
    tasks.append(
        make_task(
            ...
            r"""A clinic tracks a throughput index $f$ (brown) and its derivative $f'$ (green) on one shared figure. Which of the following statements is/are correct?""",
            [
            ...
            [True, True, False, True, False],
            [
                r"""Critical points of $f$ are exactly the zeros of $f'$; the figure shows those alignments""",
                r"""Positive $f'$ means $f$ is increasing — brown climbs wherever green is above the axis""",
...
Change E to: brown has a local maximum near x=1.""",
    ...
    tasks.append(
        make_task(
            ...
            [True, True, False, True, False],
            [
                r"""Critical points of $f$ are zeros of $f'$; the marked green crossings sit under brown's turning points""",
                r"""Positive $f'$ means $f$ is increasing: brown climbs wherever green is above the axis""",
            ...
            r"""Consistency between $f$ and $f'$ on one plane: zeros of green mark turns of brown; the peak of green is steepest slope, not a peak of brown.""",
            figure=fig24,
        )
    )
    ...
    tasks.append(
        make_task(
            ...
            [
                r"Marginal cost is $C'(Q)=3Q^{2}-12Q+40$.",
            ...
            [True, True, True, False, True],
            [
                r"""Differentiate term by term: $C'=3Q^{2}-12Q+40$""",
                r"""$C''(Q)=6Q-12=0$ at $Q=2$, and $C''$ changes sign there, so $Q=2$ is an inflection of $C$""",
                r"""$C''(Q)<0$ on $(0,2)$, so $C'$ is decreasing there""",
                r"""Inflection of $C$ is about $C''=0$; it need not solve $AC=MC$. Those meet where $AC'=0$, a different condition""",
                r"""$C(0)=10$ is the fixed cost""",
            ],
            r"""Read $C'$, $C''$, and fixed cost separately. The inflection $C''=0$ at $Q=2$ marks where MC stops falling; it is not the $AC=MC$ point.""",
        )
    )
    ...
    tasks.append(
        make_task(
            ...
            [
                ...
                r"""$P'<0$ on $(-\infty,3)$ and $P'>0$ on $(3,\infty)$, so profit is increasing for $x>3$""",
                r"""$P'$ changes $-$ to $+$ at $x=3$, and $P''(3)=2>0$, so $x=3$ is a local minimum""",
            ],
            r"""Simplify the Newton quotient to recover $P'=2x-6$. Evaluate the tangent carefully with the point-slope form. The critical point $x=3$ is a minimum; profit rises afterward.""",
        )
    )
    ...
    tasks.append(
        make_task(
            ...
            [
                r"Marginal cost is $C'(Q)=40/\sqrt{Q}$.",
    ...
    tasks.append(
        make_task(
            ...
            [True, True, True, True, False],
            [
                r"""$\pi'(t)=16-2t=0$ at $t=8$, and $\pi''=-2<0$""",
    ...
    tasks.append(
        make_task(
            ...
            [
                ...
                r"$S''(1)<0$ and $S''(3)>0$, matching the local max / local min reading.",
            ],
            [True, True, True, False, True],
            [
                r"""$S'=3t^{2}-12t+9=3(t^{2}-4t+3)=3(t-1)(t-3)$""",
                r"""At $t=1$, $S'$ changes $+$ to $-$ (for $t<1$ both factors negative so $S'>0$; between $1$ and $3$, $S'<0$)""",
                r"""At $t=3$, $S'$ changes $-$ to $+$""",
                ...
                r"""$S''=6t-12$, so $S''(1)=-6<0$ and $S''(3)=6>0$""",
            ],
            r"""Factor $S'$ to locate $t=1$ (local max) and $t=3$ (local min). Endpoint comparison shows $S(4)=S(1)=4$, so the closed-interval global max is not unique to $t=1$.""",
        )
    )
