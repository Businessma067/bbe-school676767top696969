...
DEEP_BODIES = {
    "MATH 11.161": [
        r"""Profit is revenue minus cost. Interior candidates solve $P'=0$; a sign chart of $P'$ or the sign of $P''$ then classifies a local max versus a local min.
...
The derivation therefore disagrees with the claim.""",
        r"""Average cost is total cost divided by output. Form $AC=\dfrac{C}{Q}$ before differentiating; the sign of $AC'$ says whether a little more output raises or lowers cost per unit.
...
The fixed-cost term $\dfrac{F}{Q}$ in $AC$ produces the $-\dfrac{F}{Q^{2}}$ piece of $AC'$; that is why average cost can fall even when marginal cost is positive.
...
With equal levels at the left endpoint $a$, $\int_a^b\bigl(R_e'(Q)-R'(Q)\bigr)\,dQ=R_e(b)-R(b)$ inherits the sign of $R_e'-R'$ on $(a,b)$.
...
The derivation therefore agrees with the claim.""",
        r"""Point price elasticity is $\varepsilon=\dfrac{D'(p)\,p}{D(p)}$. For ordinary downward-sloping demand, revenue peaks where $\varepsilon=-1$, which is equivalent to $MR=0$ on the quantity side.
...
Falling $AC$ and $MC$ can coexist with rising total cost because $C'=MC>0$; never confuse a falling average with a falling total.
...
Multiplying $TC'=0$ by $Q^{2}$ isolates $Q^{2}=\dfrac{KD}{h/2}$; at that EOQ the ordering and holding cost pieces are equal.
...
Because $Q$ is concave in advertising spend, $\pi'$ is strictly decreasing and crosses zero at most once, so the critical point is the unique interior maximum.
...
The worked FOC does not match the claim.""",
    ...
    "MATH 11.178": [
        r"""EOQ balances a falling ordering-cost term against a rising holding-cost term. Set $TC'=0$, check $TC''>0$, and compare nearby order sizes by evaluating $TC$ directly.
...
The worked FOC does not match the claim.""",
    ...
    "MATH 11.180": [
        r"""Marginal utility is $U'$; risk aversion for a smooth $U$ is the statement $U''<0$. A mean-preserving spread then lowers expected utility by Jensen's inequality.
...
The worked FOC does not match the claim.""",
    ...
    "MATH 11.184": [
        r"""When a figure plots $f$, $f'$, and $f''$ together, zeros of $f'$ mark turning points of $f$, while zeros of $f''$ with a sign change mark inflections. The peak of $f'$ is steepest climb of $f$, not a peak of $f$ itself.
...
An inflection of total cost is where $C''$ changes sign — equivalently where $MC$ has a turn — and that need not be where $AC=MC$.
...
The worked FOC matches the claim.""",
    ...
    "MATH 11.186": [
        r"""The Newton quotient $\dfrac{P(a+h)-P(a)}{h}$ is the average slope between $a$ and $a+h$. Expand $P(a+h)$, cancel $P(a)$, divide by $h$, then let $h\to 0$ to recover $P'(a)$.
...
The worked FOC matches the claim.""",
    ...
    "MATH 11.191": [
        r"""Two marginal-profit curves on one figure are compared by sign and by integration when profit levels start equal. A crossing height is a common $P'$ value, not a level of total profit.
...
Those steps confirm the claim.""",
        r"""Brown and green on the figure are average and marginal cost. Falling $AC$ or $MC$ does not mean total cost falls: $C'=MC>0$ still holds. Compare heights and identities such as $MC=\dfrac12 AC$.
...
DEEP_OVERVIEWS = {
    ...
    'MATH 11.176': "Chain-rule revenue $R=100\\sqrt{a}$ against linear advertising cost peaks at $a=2500$, where $R'=1$. Square-root response is concave, so doubling spend does not double listeners. Positive $\\pi'$ at small $a$ means advertising is still worthwhile there. Work claim by claim from derivatives, sign charts, and the formulas in the stem.",
    'MATH 11.177': "Reduce to one variable with the fencing constraint, maximise $A=120x-2x^{2}$ at $x=30$, $y=60$. The second derivative is negative, and the sign of $A'$ shows area still rises as one approaches $30$ from the left. Work claim by claim from derivatives, sign charts, and the formulas in the stem.",
    'MATH 11.178': "EOQ calculus: set $TC'=-\\dfrac{KD}{Q}^{2}+\\dfrac{h}{2}=0$ to get $Q=100$. Holding cost is $\\dfrac{hQ}{2}$. The second derivative is positive, and nearby order sizes cost more. Work claim by claim from derivatives, sign charts, and the formulas in the stem.",
    ...
    'MATH 11.180': "Positive but decreasing marginal utility ($U'>0$, $U''<0$) means risk aversion. Utility keeps rising on $(0,200)$. A mean-preserving spread reduces expected utility under concavity. Work claim by claim from derivatives, sign charts, and the formulas in the stem.",
    ...
    'MATH 11.185': "Read $C'$, $C''$, and fixed cost separately. The inflection $C''=0$ at $Q=2$ marks where MC stops falling; it is not the $AC=MC$ point. Work claim by claim from derivatives, sign charts, and the formulas in the stem.",
    'MATH 11.186': "Simplify the Newton quotient to recover $P'=2x-6$. Evaluate the tangent carefully with the point-slope form. The critical point $x=3$ is a minimum; profit rises afterward. Work claim by claim from derivatives, sign charts, and the formulas in the stem.",
    ...
    'MATH 11.190': "Factor $S'$ to locate $t=1$ (local max) and $t=3$ (local min). Endpoint comparison shows $S(4)=S(1)=4$, so the closed-interval global max is not unique to $t=1$. Work claim by claim from derivatives, sign charts, and the formulas in the stem.",
    'MATH 11.191': "Sign-read each $P'$, then integrate a marginal inequality when starts match. Crossing heights of $P'$ are not profit levels. Several claims are read from curve signs, crossings, and heights on the figure.",
    ...
    'MATH 11.196': "Constants shift levels not slopes. Newton recovers $P'$, and the separate ticket problem peaks at unit elasticity. Work claim by claim from derivatives, sign charts, and the formulas in the stem.",
    ...
    'MATH 11.200': "Infer turns of $P$ from zeros of $P'$, concavity from $P''$, and never treat an inflection as a profit maximum. Several claims are read from curve signs, crossings, and heights on the figure.",
    ...
    'MATH 11.203': "Differentiate $AC$ carefully, use the sign of $AC'$ for falling averages, integrate rival MR gaps, and link $AC'=0$ to $MC=AC$. Rival comparisons follow by integrating a marginal inequality when levels match at one endpoint.",
