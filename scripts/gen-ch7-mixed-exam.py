...
def balance(claims: list[Claim]) -> list[Claim]:
    """Only repair the illegal 0-True case; 1–5 Trues are all allowed."""
...
def retarget_spec(spec: TaskSpec, target: int) -> TaskSpec:
    ...
    letters = "ABCDE"
    cur = sum(ab(c.truth) for c in claims)
    # Prefer flipping from the end
    while cur > target:
        for i in range(4, -1, -1):
            if ab(claims[i].truth):
...
def main() -> None:
    ...
    for s in specs:
        # After retarget, skip sympy check mismatches on intentionally flipped traps
        pass
    ...
    assert all(truths.get(k, 0) == 6 for k in range(1, 6)), truths
