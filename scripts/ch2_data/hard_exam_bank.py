...
# ---------------------------------------------------------------------------
# Formatting — MATH 13.18 voice (see style_1318.py)
# ---------------------------------------------------------------------------

from style_1318 import D, S, explain, join, step
...
def _lcd_chain(seed: int) -> Claim:
    ...
    body = explain(
        "The claim is an identity away from $x=\\pm 1$. Put every term over the common "
        "denominator $(x-1)(x+1)=x^2-1$, and rewrite $\\dfrac{1}{1-x}$ as $-\\dfrac{1}{x-1}$ "
        "before combining like pieces.",
        [
            (
...
def _rational_product_chain(seed: int) -> Claim:
    ...
    body = explain(
        f"The claim chains three monomial fractions in ${x}$, ${y}$, and ${z}$. "
        "Division by a fraction is multiplication by its reciprocal; only then can "
        "common factors be cancelled in one pass.",
        [
            (
...
def _signed_square_monomial(seed: int) -> Claim:
    ...
    stmt = f"For ${var}\\neq 0$, ${lhs}={rhs}$."
    body = explain(
        f"The quotient has a squared monomial in the denominator. Square that factor first — "
        f"the minus sign cannot survive inside $(-{k}{var})^2$.",
        [
            (
                "Square the denominator",
                rf"(-{k}{var})^2={k**2}{var}^2",
                f"The denominator becomes a positive power of ${var}$.",
            ),
            (
                "Reduce the quotient",
                sp.latex(val),
...
def _diff_sq_neg_exp(seed: int) -> Claim:
    ...
    body = explain(
        "The product is a difference of squares. Set "
        f"$A={c}{var}^{{-1}}$ so $(A-1)(A+1)=A^2-1$, then expand the reciprocal square.",
        [
            (
                "Apply $A^2-1$",
                rf"({c}{var}^{{-1}})^2-1=\frac{{{c*c}}}{{{var}^2}}-1",
                f"The reciprocal square carries coefficient ${c*c}$ on ${var}^2$ in the denominator.",
            ),
        ],
        (
            f"The printed coefficient ${c*c}$ is the one that survives the expansion."
            if truth
...
def _complex_fraction_factor(seed: int) -> Claim:
    ...
    body = explain(
        f"The stacked quotient mixes a difference of squares with a linear factor. "
        f"Factor every polynomial piece before cancelling any $(x\\pm 2)$ factor.",
        [
            (
...
def _nested_radical_exponents(seed: int) -> Claim:
    ...
    body = explain(
        f"Every radical in the quotient must be written as a fractional exponent before "
        f"powers are added or subtracted. Work the numerator and denominator separately.",
        [
            (
                "Numerator",
                rf"{var}^{{2/3}}\cdot({var}\cdot{var}^{{1/2}})^{{1/3}}"
                rf"={var}^{{2/3}}\cdot{var}^{{(1+1/2)/3}}={var}^{{2/3}}\cdot{var}^{{1/2}}",
                f"Inside the cube root, ${var}\\cdot\\sqrt{{{var}}}$ becomes a single power of ${var}$.",
            ),
            (
                "Denominator",
...
def _given_relation_then_value(seed: int) -> Claim:
    ...
    body = explain(
        f"The given relation ${a_name}^{{-3}}{b_name}^2=2$ does not determine the claim by "
        f"inspection alone. Set $R={a_name}^{{-3}}{b_name}^2$, rewrite each summand in terms "
        f"of $R$, and only then substitute $R=2$.",
        [
            (
                "First summand",
                rf"\frac{{{a_name}^8{b_name}^3}}{{{a_name}^2{b_name}^7}}={a_name}^6{b_name}^{{-4}}=R^{{-2}}",
                f"Powers of ${a_name}$ and ${b_name}$ collapse to a negative power of $R$.",
            ),
            (
                "Second summand",
            ...
            (
                "Substitute $R=2$",
                r"2^{-2}+3\cdot 2^{4}=\tfrac{1}{4}+48=\tfrac{193}{4}=48.25",
...
def _inequality_square(seed: int) -> Claim:
    ...
    if truth:
        ...
        body = explain(
            f"The inequality compares two quadratic-looking sides in ${a}$ and ${b}$. "
            "Bring every term to one side and factor before deciding whether the "
            "difference is always nonnegative.",
            [
                (
        ...
        body = explain(
            f"The printed inequality looks like a square identity, but the middle signs differ. "
            f"Rearrange and factor before testing whether the difference keeps one sign.",
            [
                (
                    "Difference of the two sides",
                    rf"{a}^2+{a}{b}-({a}{b}+{b}^2)={a}^2-{b}^2=({a}-{b})({a}+{b})",
                    f"The factorisation $({a}-{b})({a}+{b})$ is not a square, so the sign can change.",
                ),
            ],
            f"For ${a}=0$, ${b}=1$ the left side is $0$ and the right side is $1$, "
            f"so the inequality fails on that pair.",
        )
...
def _text_reciprocal_sum(seed: int) -> Claim:
    ...
    body = explain(
        f"The statement is entirely worded. Translate each English phrase into symbols "
        f"for ${var}\\neq 0$ before comparing the two sides.",
        [
            (
                "Left-hand wording",
                rf"\frac{{2}}{{{var}+\frac{{1}}{{{var}}}}}=\frac{{2{var}}}{{{var}^2+1}}",
                f"Twice the reciprocal of ${var}+\\dfrac{{1}}{{{var}}}$ clears to a single rational expression.",
            ),
            (
                "Right-hand wording",
                rhs_check,
...
def _text_abs_distance(seed: int) -> Claim:
    ...
    if truth:
        ...
        body = explain(
            ...
            f"On that closed interval each absolute value opens without a minus sign.",
            [
                (
                    "Remove the bars inside the segment",
                    rf"|{var}-{lo}|+|{var}-{hi}|=({var}-{lo})+({hi}-{var})={span}",
                    f"The ${var}$ terms cancel, leaving the constant segment length ${span}$.",
                ),
            ],
            f"The sum equals the length ${span}$ throughout $[{lo},{hi}]$.",
        )
    else:
        stmt = (
        ...
        body = explain(
            ...
            f"before accepting a constant equal to the segment length.",
            [
                (
                    f"For ${var}>{hi}$",
                    rf"|{var}-{lo}|+|{var}-{hi}|=({var}-{lo})+({var}-{hi})=2{var}-({lo}+{hi})",
                    f"The result still depends on ${var}$, not on the fixed length ${span}$ alone.",
                ),
            ],
            f"A point outside $[{lo},{hi}]$ already disproves the universal constant claim.",
        )
...
def _text_sophie(seed: int) -> Claim:
    ...
    if truth:
        ...
        body = explain(
            f"The verbal claim is a Sophie Germain factorisation in ${a}$ and ${b}$. "
            f"Insert $\\pm 4{a}^2{b}^2$ to create a difference of squares, then factor.",
            [
                (
        ...
        body = explain(
            f"The wording stops at a single square. Expand ${a}^2+2{b}^2$ and compare "
            f"with ${a}^4+4{b}^4$ before accepting the verbal equality.",
            [
                (
                    "Expand the printed square",
                    rf"({a}^2+2{b}^2)^2={a}^4+4{a}^2{b}^2+4{b}^4",
                    f"The cross term $4{a}^2{b}^2$ is not present in ${a}^4+4{b}^4$.",
                ),
            ],
            f"The Sophie Germain rewrite must subtract $(2{a}{b})^2$ after adding it — "
            f"the wording stops one step too early.",
        )
...
def _text_partial_fraction(seed: int) -> Claim:
    ...
    if truth:
        ...
        body = explain(
            f"The words describe a partial-fraction split of $\\dfrac{{{coeff}}}{{x^2-1}}$. "
            "Translate into symbols, then clear the proposed difference of fractions.",
            [
                (
        ...
        body = explain(
            f"The verbal decomposition puts the full numerator ${coeff}$ on each partial "
            f"fraction. Clear that difference and compare with $\\dfrac{{{coeff}}}{{x^2-1}}$.",
            [
                (
...
def _text_exponent_stack(seed: int) -> Claim:
    ...
    if truth:
        ...
        body = explain(
            f"The claim describes a tower of powers on positive ${var}$. "
            "Translate each English step into an exponent, multiplying powers inside out.",
            [
                (
                    "Inner power",
                    rf"({var}^{{2}})^{{3}}={var}^{{6}}",
        ...
        body = explain(
            f"Power of a power multiplies exponents; it does not add $2+3$ before the root. "
            f"Reduce the tower on positive ${var}$ first.",
            [
                (
                    "Correct tower",
                    rf"(({var}^{{2}})^{{3}})^{{1/2}}={var}^{{3}}",
...
def _lcd_sum_trap_must_finish(seed: int) -> Claim:
    ...
    if truth:
        ...
        body = explain(
            ...
            f"denominator is ${x}+{y}$ instead of ${x}{y}$. Clear with the product denominator first.",
            [
                (
                    "Correct combination",
                    rf"\frac{{{n1}}}{{{x}}}+\frac{{{n2}}}{{{y}}}"
                    rf"=\frac{{{n1}{y}+{n2}{x}}}{{{x}{y}}}",
                    f"Only ${x}{y}$ is the common denominator for unrelated linear factors.",
                ),
            ],
...
def _abs_factor_must_finish(seed: int) -> Claim:
    ...
    if truth:
        ...
        body = explain(
            f"The absolute value of a factored quadratic must be read through $|UV|=|U||V|$. "
            f"Factor first, then split the bars across the linear factors.",
            [
                (
                    "Factor the trinomial",
                    rf"x^2-{s}x+{prod}=(x-{p})(x-{q})",
        ...
        body = explain(
            f"The polynomial factorisation $x^2-{s}x+{prod}=(x-{p})(x-{q})$ is correct, "
            f"but absolute value is not the same as dropping the bars on a signed product.",
            [
                (
