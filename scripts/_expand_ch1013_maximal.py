...
def normalize_header_closer(text: str, letter: str, is_true: bool) -> str:
    ...
    # Strip full verdict phrases only (do not leave dangling "so")
    body = re.sub(
...
def inject_ch13_p_recovery(expl: str, overview: str) -> str:
    ...
    for d in displays:
        ...
        if not recovery_steps:
            if re.search(r"\bp\b|E\[|\\mathrm\{Var\}|\\dfrac|\\frac", ds):
                recovery_steps.append(ds)
            continue
        # Continue the recovery chain (leading "=" steps / further p formulas)
        if ds.startswith("=") or re.search(r"\bp\b|E\[|\\mathrm\{Var\}|\\dfrac|\\frac", ds):
            recovery_steps.append(ds)
        else:
            break
...
def main() -> None:
    ...
    if not only or "math-ch11-differentiation.ts" in only:
        path = ROOT / "src/data/math-ch11-differentiation.ts"
        total, changed = expand_ts_file(path)
        print(f"{path.name}: total={total} changed={changed}")
