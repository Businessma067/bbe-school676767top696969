...
def build_all() -> list[dict]:
    ...
    # ======================================================================
    # 27. NESTED  (5 true)  g ∘ f^{-1}
    # ======================================================================
    nest27 = expand((x + 2) ** 2 - 1)
    assert nest27 == expand(x ** 2 + 4 * x + 3)
    assert ev(nest27, -1) == 0 and ev(nest27, -3) == 0
    assert vertex_of(nest27) == (-2, -1)
    add(
        kind="nested",
        title="Parabola after an inverse shift",
        context=(
            r"Let $f(x)=x-2$ (so $f^{-1}(x)=x+2$) and $g(x)=x^{2}-1$. "
            r"Study $g(f^{-1}(x))$"
        ),
        statements=[
            r"The nested rule $g(f^{-1}(x))$ equals $(x+2)^{2}-1$, which is $g$ evaluated at the inverse shift $x+2$.",
            r"Expanded, that nested rule is $x^{2}+4x+3$, whose constant term $3$ equals $g(f^{-1}(0))$.",
            r"The nested rule vanishes at $x=-1$ and at $x=-3$, so Vieta's sum of those roots is $-4$, matching the middle coefficient.",
            r"At $x=0$ the nested rule equals $3$, which is also $g(f^{-1}(0))=g(2)$.",
            r"The nested rule opens upwards, and its vertex is $(-2,-1)$.",
        ],
        key=[True, True, True, True, True],
        expls=[
            pack("A", True, [
                "Replace the input of $g$ by the inverse shift $x+2$.",
                D(r"f^{-1}(x)=x+2"),
                D(r"g(f^{-1}(x))=(x+2)^{2}-1"),
                close(True, "The nested rule is $g$ evaluated at the inverse shift"),
            ]),
            pack("B", True, [
                "Expand the completed square.",
                D(r"(x+2)^{2}-1=x^{2}+4x+4-1"),
                D(r"x^{2}+4x+3"),
                D(r"g(f^{-1}(0))=g(2)=3"),
                close(True, "The expansion is $x^{2}+4x+3$, and the constant term matches $g(f^{-1}(0))$"),
            ]),
            pack("C", True, [
                "Set the completed square equal to zero, or factor the expansion.",
                D(r"(x+2)^{2}=1"),
                D(r"x=-1\qquad x=-3"),
                D(r"(-1)+(-3)=-4"),
                close(True, "The two zeros sum to $-4$, matching the middle coefficient"),
            ]),
            pack("D", True, [
                "Evaluate the nested rule at the origin, and also walk through the two maps.",
                D(r"(0+2)^{2}-1=3"),
                D(r"f^{-1}(0)=2"),
                D(r"g(2)=4-1=3"),
                close(True, "Both routes give the nested value $3$ at $x=0$"),
            ]),
            pack("E", True, [
                "The leading coefficient of $x^{2}+4x+3$ is $1>0$, so the arms open upwards. Completing the square places the vertex at the shift.",
                D(r"(x+2)^{2}-1"),
                D(r"(-2,-1)"),
                close(True, "An upward parabola with vertex $(-2,-1)$"),
            ]),
        ],
        overview=ov(
            r"$g(f^{-1}(x))=(x+2)^{2}-1=x^{2}+4x+3$. Zeros $-1,-3$; vertex $(-2,-1)$; value $3$ at $x=0$."
        ),
    )
